...
class ClockViewController: UIViewController, CAAnimationDelegate {
	...
	var myClockBackground: UIClockBackground!
    ...
    override func viewDidLayoutSubviews() {
		if self.taskDetail == nil && self.containerView != nil {
			self.view.bringSubview(toFront: myClock)
...
//		}
		...
		var index = 0
		var i = 0
		let convertedFrame = self.view.convert(self.gradientView.frame, to: self.gradientView)
		timer = Timer.scheduledTimer(withTimeInterval: 0.02, repeats: true) { (_)  in
			let sector = UICircularGradientSector(frame: convertedFrame, partition: partition, index: index, innerIndex: i)
			self.gradientView.addSubview(sector)
			if i == partition - 1 {
				i = 0
				if index == 11 {
					timer.invalidate()
