...
//        print(dbPath)
        ...
        if sqlite3_step(stmt) == SQLITE_DONE {
            sqlite3_close(dbpointer)
            //operation = self.client?.syncContext?.push()
            return true
        } else {
    ...
    func fetchUserInfoFromLocalDB(userId: Int64 = -1 ) throws -> [Any] {
        ...
        let selectQueryString = "SELECT zid, zusername, zpassword, zemail, zlast_update FROM ZMS_User WHERE zid= '" + String(userId) + "'"
    ...
    func userAuthentication(email: String, password: String ) -> Int64 {
        ...
        //SQL command for fecting a row from database base on id
        var selectQueryString = "SELECT zid, zemail FROM ZMS_User WHERE zpassword= '" + password + "' AND " +
            "zemail = " + "'" + email + "'"
        var stmt: OpaquePointer?
        sqlite3_prepare(dbpointer, selectQueryString, -1, &stmt, nil)
    ...
    func validateUserEmailOnline(email: String, onlineDB: Bool) -> Bool {
        if(onlineDB)
        {
            ...
            self.userTable = self.client!.syncTable(withName: "ZMS_User")
            let predicate = NSPredicate(format: "zemail == %@",email)
            let queryString = self.userTable?.query(with: predicate)
            print("validating user email")
            print(queryString)
            // MSSync table is only for pulling and pushing operations only
            let res = self.userTable?.pull(with: queryString, queryId: "AllRecords"){
                (error) in
                if error != nil {
                    print("Error when pulling from online database ", error)
                }
            }
    ...
    func UpdateOnlineDBFromLocalDB() -> Bool{
        ...
        self.initOnlinedatabase()
        print(self.client?.syncContext)
        operation = self.client?.syncContext?.push()
        print(operation)
        print("pushed to the online database")
        return true
    }
